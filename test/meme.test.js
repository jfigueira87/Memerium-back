...
import { getMemes, getMeme, createMeme, updateMeme, deleteMeme } from '../controllers/memeController.js';
...
describe('GET /memes', () => {
    it('should return 200 and a list of memes', async () => {
      const mockMemes = [{ id: 1, name: 'Meme1' }, { id: 2, name: 'Meme2' }];
    ...
    it('should return 500 if there is an error', async () => {
      ...
      expect(response.body).toEqual({ error: 'Ha ocurrido un error mientras se encontraba memes.' });
    });
  });
...
describe('Meme Controller', () => {
  describe('GET /memes/:id', () => {
    ...
    it('should return 500 if there is an error', async () => {
      ...
      expect(response.status).toBe(500);
      expect(response.body).toEqual({ error: 'Ha ocurrido un error mientras se encontraba un meme.' });
    });
  });

  describe('POST /memes', () => {
    it('should create a new meme and return it', async () => {
      const mockMeme = { id: 1, title: 'Meme1', category: 'Category1', tags: ['tag1'], url: 'http://example.com' };
      memeModel.findOne.mockResolvedValue(null);
    ...
    it('should return 500 if there is an error', async () => {
      ...
      const response = await request(app).post('/memes').send({ title: 'Meme1', category: 'Category1', tags: ['tag1'], url: 'http://example.com' });

      expect(response.status).toBe(500);
      expect(response.body).toEqual({ error: 'Error creando meme', details: 'Database error' });
    });
  });

  describe('PUT /memes/:id', () => {
    it('should update the meme and return it', async () => {
      const mockMeme = { id: 1, title: 'Updated Meme', category: 'Updated Category', tags: ['updatedTag'], url: 'http://updated.com' };
    ...
    it('should return 404 if the meme is not found', async () => {
      ...
      const response = await request(app).put('/memes/1').send({ title: 'Updated Meme', category: 'Updated Category', tags: ['updatedTag'], url: 'http://updated.com' });
    ...
    it('should return 500 if there is an error', async () => {
      ...
      expect(response.status).toBe(500);
      expect(response.body).toEqual({ error: 'Error actualizando meme', details: 'Database error' });
    });
  });
  ...
  describe('DELETE /memes/:id', () => {
    ...
    it('should return 500 if there is an error', async () => {
      ...
      expect(response.status).toBe(500);
      expect(response.body).toEqual({ error: 'Error borrando meme', details: 'Database error' });
    });
  });
});
